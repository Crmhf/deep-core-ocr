...
**Deep Core OCR** 是一个面向纸质文档、桌面截图、实物影像、网页 UI、证件票据等多场景的 AI 图像识别技能。它整合多个主流多模态 / OCR 模型（DeepSeek-OCR、Qwen2.5-VL、MiniMax-M3），提供统一的图像识别入口、自动降级机制、场景化选型指南和结构化输出能力。
...
本技能虽然名为 OCR，但实际调用的模型可分为两类：

### 纯 OCR 模型

**核心能力**：把文字从图片里“抠”出来，专注于文字提取、版面还原、标点识别。

- **代表模型**：DeepSeek-OCR
- **适用场景**：印刷体文档、票据、规范手写体、网页文案等以文字提取为主的任务
- **特点**：速度快、成本低、对文字排版敏感，但对图像中的物体、场景、语义关系理解有限

### 视觉理解模型

**核心能力**：能“看懂”整张图，包括物体、场景、人物关系、空间布局、画面语义；OCR 只是其能力子集。

- **代表模型**：Qwen2.5-VL、MiniMax-M3
- **适用场景**：实物影像、复杂界面截图、需要理解画面上下文再做文字提取的任务
- **特点**：通用性强，能处理倾斜、遮挡、杂乱背景；MiniMax-M3 在复杂实景和多语言上能力更强

### 选型原则

```
任务只需要文字 → DeepSeek-OCR（纯 OCR）
任务需要理解图片再提取文字 → Qwen2.5-VL / MiniMax-M3（视觉理解）
复杂实景 / 兜底 → MiniMax-M3
```
...
| 场景类型 | 任务类型 | 示例 | 推荐模型 |
|---------|---------|------|---------|
| 纸质文档数字化 | 纯 OCR | 合同、票据、报表、档案、手写单据 | DeepSeek-OCR / Qwen2.5-VL |
| 桌面截图信息提取 | OCR + 视觉理解 | 客户端界面、后台系统、弹窗报错、日志 | Qwen2.5-VL / DeepSeek-OCR |
| 实物影像识别 | 视觉理解 + OCR | 证件、铭牌、标签、商品外包装 | Qwen2.5-VL / MiniMax-M3 |
| 网页 UI 合规校验 | 纯 OCR | 落地页文案、弹窗提示、底部声明 | DeepSeek-OCR / Qwen2.5-VL |
| 证件 / 发票 / 表格录入 | OCR + 版式理解 | 身份证、发票、收据、报表 | Qwen2.5-VL / MiniMax-M3 |

## 模型矩阵

| 模型 | 类型 | 能力档位 | 成本 | 速度 | 主用场景 | 当前状态 |
|------|------|---------|------|------|---------|---------|
| **DeepSeek-OCR** | 纯 OCR | ⭐⭐⭐⭐ A 级 | 低 | 快 | 印刷体文档、票据、规范手写体 | ✅ 可用（使用有效 key） |
| **Qwen2.5-VL** | 视觉理解 | ⭐⭐⭐⭐ A 级 | 低 | 中 | **日常主力、通用识别、版式理解** | ✅ 可用（使用有效 key） |
| **MiniMax-M3** | 视觉理解 | ⭐⭐⭐⭐⭐ S 级 | 中 | 中 | 复杂图片、实景、多语言、兜底 | ✅ 可用 |
...
```
只需要提取文字（印刷体 / 文档 / 网页文案）→ DeepSeek-OCR（纯 OCR）
需要理解图片后再提取文字（截图 / 实物 / 版式）→ Qwen2.5-VL（视觉理解）
复杂实景 / 多语言 / 兜底 → MiniMax-M3（视觉理解）
```

### 推荐优先级

1. **先判断是“纯 OCR”还是“视觉理解”任务**
2. **纯 OCR 任务优先用 DeepSeek-OCR**：成本低、速度快
3. **需要理解画面上下文时用 Qwen2.5-VL**：通用性强
4. **复杂场景或前两者失败时用 MiniMax-M3 兜底**
